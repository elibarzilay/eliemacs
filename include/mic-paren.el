...
;; Slightly edited by Eli Barzilay (moved vars, remove XEmacs code)
...
;;    )
;;   )
(defalias 'mic-make-overlay 'make-overlay)
(defalias 'mic-delete-overlay 'delete-overlay)
(defalias 'mic-overlay-put 'overlay-put)
(defalias 'mic-cancel-timer 'cancel-timer)
(defalias 'mic-run-with-idle-timer 'run-with-idle-timer)
...
;;     1))
(defalias 'mic-char-bytes 'char-bytes)
...
;; Function defined by mic-paren to be compatible with multibyte Emacses."
;;     (char-after (1- pos))))
(defalias 'mic-char-before 'char-before)
