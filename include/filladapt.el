...
(defun filladapt-fill-paragraph (function &rest args)
  (catch 'done
		...
		(progn
		  (if filladapt-fill-column-tolerance
		      (let* ((low (- fill-column
			     ...
			     (delta 0))
			(while (not done)
			  (setq retval (filladapt-funcall* function args))
    ...
    (filladapt-funcall* function args)))
...
and initial semicolons."
  (interactive "P")
  (let ((filladapt-inside-filladapt t))
    (filladapt-fill-paragraph 'lisp-fill-paragraph justify)))
...
space does not end a sentence, so don't break a line there."
  ...
  ;; ensure beg <= end as in core version of fill-region
  (let ((newbeg (min beg end))
        (newend (max beg end)))
    (setq beg newbeg)
    (setq end newend))
...
(defun filladapt-funcall* (function args)
  "Call the old definition of a function that filladapt has usurped."
  (apply (cdr (assoc function filladapt-function-table)) args))
