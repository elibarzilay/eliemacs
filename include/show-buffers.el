...
(setq bs-attributes-list
      '((""        1  1 left  bs--get-marked-string)
        ("RM"      2  2 left  eli-bs--get-modified/readonly-string)
        (""        1  1 left  "|")
        ("Buffer" 30 30 left  eli-bs--get-name)
        (""        1  1 left  " ")
        ("Size"    5  5 right eli-bs--get-size-string)
        (""        1  1 left  " ")
        ("Mode"   14 14 right eli-bs--get-mode-name)
        (""        1  1 left  " ")
        ("File"    0 12 left  eli-bs--get-file-name)))

(defun eli-bs--get-modified/readonly-string (_start-buffer _all-buffers)
  (let ((pr (lambda (str) (propertize str 'font-lock-face (eli-bs--get-face))))
        (modified (buffer-modified-p))
        (readonly buffer-read-only))
    (cond ((and readonly modified) (funcall pr "%*"))
          (readonly (if buffer-file-name (funcall pr "%%")
                        (concat (funcall pr "%") " ")))
          (modified (if buffer-file-name (funcall pr "**")
                        (concat " " (funcall pr "*"))))
          (t "  "))))
