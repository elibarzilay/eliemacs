...
(defvar calculator-prev-buffer nil
  "The buffer that was active before we start, if using electric mode.")
  ...
  (if calculator-electric-mode
    (save-window-excursion
      ...
      (let (old-g-map old-l-map (echo-keystrokes 0)
        ...
        (setq calculator-prev-buffer (window-buffer (minibuffer-window)))
...
(defun calculator-number-to-string (num)
  ...
  (cond
    ((and (numberp num) (<= 1.0e+INF num)) "inf")
    ...
    (if (or (stringp inp) (not (arrayp inp)))
      ...
      (let* ((i -1) (converted-str (make-string (length inp) ? )) k)
        ...
        (while (< (setq i (1+ i)) (length inp))
          ...
          (and (fboundp 'key-press-event-p) (key-press-event-p k)
               (fboundp 'event-key) (event-key k)
               (setq k (or (event-key k) k)))
...
(defun calculator-quit ()
  ...
  (if calculator-electric-mode
    (and calculator-prev-buffer
         (progn (set-window-buffer (minibuffer-window) calculator-prev-buffer)
                (setq calculator-prev-buffer nil)))
    (condition-case nil
